...
A look into income quarantines across the various occupation groups of interest.

```{r mh-inc-og}

all_og <- data_household_incomes$OCCUPATION_GROUP|>
  unique()

cbind(get_inc_quantiles(level = "state",
                        state = "Maharashtra"),
get_inc_quantiles(level = "state",
                        state = "Maharashtra", 
                  with_ur_lens = "yes"))|>
  select(-3)|>
  gt::gt()|>
  gt::tab_options(heading.subtitle.font.size = 8)|>
  gt::cols_align(align = "center")|>
  gt::tab_header(title = "Table 9 : Maharashtra Income Quantiles") 

```
```{r def-fun-incq-og-mh}

## function to get all income quantile of an Occupation Group for any state

get_og_incq <- function(ocg, 
                        state, 
                        with_ur_lens = "no"){
  ...
  if(with_ur_lens == "no"){
    ...
    tibble(
      quantile = c("0%","25%","50%","75%","100%"),
      "{{state}}_{{ocg}}_rural_inc" := sat_rural_inc_q,
      "{{state}}_{{ocg}}_urban_inc" := sat_urban_inc_q
    )
  }
...
purrr::pmap_dfc(
  .l = list(
    ocg = all_og,
    state = rep("Maharashtra",19),
    with_ur_lens = rep("no",19)
  ),
  .f = get_og_incq
)|>
  janitor::clean_names()|>
  rename(Quantiles = quantile_1)|>
  select(!tidyselect::contains("quantile_"))|>
  pivot_longer(cols = -Quantiles,names_to = "occupation_grp",values_to = "inc_q")|>
  pivot_wider(names_from = Quantiles,
              values_from = inc_q)|>
  mutate(
    occupation_grp = stringr::str_remove(
      occupation_grp,"maharashtra_"
    )
  )|>
  janitor::clean_names()|>
  arrange(desc(x0_percent))|>
  gt::gt()|>
  gt::tab_options(heading.subtitle.font.size = 8)|>
  gt::cols_align(align = "center")|>
  gt::tab_header(title = "Table 10 : Maharashtra -  Income Quantiles for Occupation groups")

```

## Income from government transfers

People who report to have received transfers (direct or in kind) from Government.

```{r inc-transfers}

data_household_incomes|>
  mutate(
    received_gov_transfer = ifelse(
      INCOME_OF_HOUSEHOLD_FROM_GOVERNMENT_TRANSFERS > 0|
        INCOME_OF_HOUSEHOLD_FROM_IN_KIND_TRANSFERS_FROM_GOVERNMENT > 0,
      1,0
    ),
    received_gov_transfer_direct = ifelse(
      INCOME_OF_HOUSEHOLD_FROM_GOVERNMENT_TRANSFERS > 0,
      1,0
    ),
    received_gov_transfer_inkind = ifelse(
      INCOME_OF_HOUSEHOLD_FROM_IN_KIND_TRANSFERS_FROM_GOVERNMENT > 0,
      1,0
    )
  )|>
  summarise(
    perc_received_gov_transfer = Mean(
      received_gov_transfer,
      weights = w_cntry),
    perc_received_gov_transfer_direct = Mean(
      received_gov_transfer_direct,
      weights = w_cntry),
    perc_received_gov_transfer_inkind = Mean(
      received_gov_transfer_inkind,
      weights = w_cntry)
  )|>
  pivot_longer(cols = everything(),names_to = "GOV Transfer",values_to = "Percentage HH")|>
  mutate(
    `Percentage HH` = round(`Percentage HH`*100,2)
  )|>
  gt::gt()

```

```{r inc-transfer-state}
data_household_incomes|>
  mutate(
    received_gov_transfer = ifelse(
      INCOME_OF_HOUSEHOLD_FROM_GOVERNMENT_TRANSFERS > 0|
  ...
  arrange(desc(perc_received_gov_transfer))|>
  gt::gt()
```
